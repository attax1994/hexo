---
title: Vue源码解析05--core.util.props
date: 2019-04-26 18:31:05
categories: 
- 前端开发
tags: 
- 技术原理
...
props.js主要暴露一个`validateProp()`方法，将vm中的单个Prop合法化。如遇到不合法的地方，则发出警告。
...
需要注意，如果`type`是`Object`或`Array`，就必须使用工厂模式来返回初始值，即`default`为一个Function
...
检查输入的值是否符合`type`中声明的类型，以及能否通过定制的`validator`
