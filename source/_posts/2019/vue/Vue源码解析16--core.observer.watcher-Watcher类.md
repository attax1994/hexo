---
title: Vue源码解析16--core.observer.watcher-Watcher类
date: 2019-04-26 18:31:16
categories: 
- 前端开发
tags: 
- 技术原理
...
`Deps`中添加一个依赖。先暂存入`newDeps`做查重，没有重复的情况下，将该`watcher`放入`dep.subs`订阅者中
...
`dep`中添加这个watcher依赖，只会被computed属性的watcher调用。
...
停用这个watcher，清除这个watcher的所有引用，包括`vm._watchers`和`dep.subs`中的。
